...
export function HeroSection() {
  return (
    <section className="relative mx-auto grid min-h-[auto] grid-cols-1 items-center gap-10 overflow-hidden pt-6 sm:pt-8 lg:min-h-[720px] lg:grid-cols-12 lg:gap-12 lg:pt-4">
      <div className="glyph-quiet-glow pointer-events-none absolute left-1/2 top-4 -z-10 h-64 w-64 -translate-x-1/2 rounded-full bg-mint/50 blur-3xl lg:left-[62%] lg:h-96 lg:w-96" />
      <motion.div className="min-w-0 lg:col-span-6" initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55 }}>
        <div className="glyph-label mb-5 inline-flex items-center gap-2 rounded-full border border-cocoa/10 bg-parchment/80 px-4 py-2 text-sm font-medium text-cocoaSoft shadow-lift">
          <span className="h-2 w-2 rounded-full bg-olive" />
          Bilingual health literacy <span aria-hidden="true">&middot;</span> Privacy-safe by design
        </div>
        <h1 className="glyph-display max-w-full text-[clamp(2.65rem,10vw,4rem)] font-extrabold leading-[1.02] tracking-tight text-cocoa sm:text-5xl lg:max-w-3xl lg:text-6xl xl:text-7xl">
          Understand common diseases clearly.
        </h1>
        <p className="mt-5 max-w-2xl text-lg leading-8 text-cocoaSoft sm:text-xl">
          Learn common diseases through simple bilingual cards, check the classifier, and earn onchain learning badges.
        </p>
        ...
        <div className="mt-7 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
          <Link href="/library" className="glyph-cta-primary focus-ring inline-flex items-center justify-center gap-2 rounded-full bg-cocoa px-6 py-4 text-sm font-semibold text-cream shadow-lift transition hover:-translate-y-0.5 hover:bg-deepBlack sm:w-auto">
        ...
        <div className="mt-8 flex flex-wrap gap-2">
          {["Education only", "Privacy-safe", "Bilingual", "Onchain learning proof"].map((item) => (
            <span key={item} className="glyph-chip rounded-full border border-cocoa/10 bg-parchment px-4 py-2 text-sm font-semibold text-cocoaSoft shadow-lift">
              {item}
            </span>
          ))}
        </div>
      ...
      <motion.div
        ...
        className="glyph-frame relative isolate min-h-[420px] overflow-hidden rounded-[2.5rem] lg:col-span-6 lg:min-h-[620px]"
      >
