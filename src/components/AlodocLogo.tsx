...
export function AlodocLogo({ compact = false }: { compact?: boolean }) {
  return (
    <Link href="/" className="flex items-center gap-3" aria-label="Alodoc home">
      <svg width="54" height="54" viewBox="0 0 54 54" fill="none" aria-hidden="true" className="shrink-0">
      ...
      {!compact && (
        <div className="leading-none">
          <div className="text-2xl font-black tracking-normal text-cocoa">Alodoc</div>
          <div className="mt-1 text-[11px] font-semibold text-cocoaSoft">Learn. Understand. Live well.</div>
        </div>
      )}
    </Link>
  );
