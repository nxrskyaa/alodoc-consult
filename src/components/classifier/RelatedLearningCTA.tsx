...
export function RelatedLearningCTA({ result, language }: { result: ClassifierResult; language: ClassifierLanguage }) {
  return (
    <motion.article initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} className="w-full min-w-0 overflow-hidden rounded-[2rem] border border-cocoa/10 bg-parchment p-4 shadow-lift sm:p-5">
      <div className="flex min-w-0 flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <div className="grid w-full min-w-0 gap-2 sm:grid-cols-2 md:max-w-sm md:shrink-0">
