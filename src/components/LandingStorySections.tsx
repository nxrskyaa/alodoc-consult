...
export function LandingStorySections() {
  return (
    <div className="grid gap-10">
        ...
        <div className="alodoc-marquee flex w-max gap-3 px-4">
          {[...Array(2)].map((_, group) =>
            ["Education only", "Privacy-safe", "Bilingual", "Onchain learning proof", "No medical records"].map((item) => (
              <span key={`${group}-${item}`} className="rounded-full bg-white px-5 py-3 text-sm font-black text-cocoaSoft shadow-lift">
                {item}
              </span>
            ))
          )}
      ...
      <section className="rounded-[2rem] border border-cocoa/10 bg-parchment p-5 shadow-soft sm:p-8">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-sm font-black uppercase text-oliveDeep">How Alodoc works</p>
            <h2 className="mt-2 text-3xl font-black text-cocoa sm:text-5xl">A soft path from learning to proof.</h2>
          </div>
          <Link href="/library" className="focus-ring inline-flex items-center justify-center gap-2 rounded-full bg-orange px-5 py-3 text-sm font-black text-white shadow-lift">
          ...
          {steps.map((step, index) => {
            ...
            return (
              <motion.div
                ...
                className="rounded-3xl bg-white p-4 shadow-lift"
              >
                <div className="grid h-12 w-12 place-items-center rounded-2xl bg-mint text-oliveDeep">
                  <Icon className="h-6 w-6" />
                </div>
                <h3 className="mt-5 text-xl font-black text-cocoa">{step.title}</h3>
                <p className="mt-2 text-sm font-semibold leading-6 text-cocoaSoft">{step.text}</p>
              </motion.div>
            );
          })}
        </div>
      ...
      <section className="grid gap-5 rounded-[2rem] border border-cocoa/10 bg-parchment p-5 shadow-soft sm:p-8 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
        <div>
          ...
          <h2 className="mt-4 text-3xl font-black leading-tight text-cocoa sm:text-5xl">Understand health numbers without storing them.</h2>
          <p className="mt-4 text-sm font-semibold leading-6 text-cocoaSoft">
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            {["Browser only", "No storage", "Education only", "ID/EN"].map((item) => (
              <span key={item} className="rounded-full bg-white px-4 py-2 text-xs font-black text-cocoaSoft shadow-lift">
                {item}
              </span>
            ))}
          </div>
          <Link href="/classifier" className="focus-ring mt-6 inline-flex min-h-[52px] items-center justify-center gap-2 rounded-full bg-orange px-5 py-3 text-sm font-black text-white shadow-lift">
      ...
      <section className="grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
        <div className="rounded-[2rem] border border-cocoa/10 bg-parchment p-5 shadow-soft sm:p-8">
          <p className="text-sm font-black uppercase text-oliveDeep">Disease quests</p>
          <h2 className="mt-2 text-3xl font-black text-cocoa sm:text-5xl">Cute visuals for serious literacy.</h2>
        ...
        <div className="grid gap-5">
          <div className="rounded-[2rem] border border-cocoa/10 bg-parchment p-5 shadow-soft sm:p-8">
            ...
            <h2 className="mt-6 text-3xl font-black text-cocoa">Health Literacy Passport</h2>
            <p className="mt-3 text-sm font-semibold leading-6 text-cocoaSoft">A warm public learning identity with XP, badge reveal, completed quests, and X avatar support.</p>
